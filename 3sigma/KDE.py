...
from AlpsNetlist import simulation
...
if __name__ == '__main__':
    ...
    design_param_num = 2
    variation_num = 4
    design_param = get_random_x(0.25)
    # 至多五轮优化
    for i in range(10):
        # 这里替换为蒙卡
        ...
        start = time.time()
        Samples = sample(design_param=design_param, mismatch_variation=mismatch_mc, name=f"get {i}th 3 sigma corner")
        ...
        # print(type(abs(simulation(design_param, corner)[0]-target)**2))
        ...
        verify = sample(design_param=optDesign, mismatch_variation=Mc_v, name=f"verify {i}th opt", verify=True)
