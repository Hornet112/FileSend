...
class OptimizationLogger:
    ...
    def __call__(self, step, variables, values):
        ...
        loss_value = -self.model.training_loss().numpy()  # 获取负对数边际似然
        self.loss_history.append(loss_value)  # 记录损失值

        print(f"Step {step}: loss = {loss_value:.4f}")
        print(f"Elapsed time: {elapsed_time:.2f}s")

        self.start_time = time.time()  # 重置时间
...
def create_model_oak(
    ...
    if optimise:
        ...
        lossresult = opt.minimize(
            model.training_loss_closure(), model.trainable_variables, options=dict(maxiter=100),   # step_callback=OptimizationLogger(model),
        )
        print("Loss history:")
        print(lossresult.fun)
        # method="BFGS"
...
class oak_model:
    ...
    def fit(
        ...
        Z = None
        # using sparse GP when size of data > 1000 大数据稀疏GP
    ...
    def get_sobol(self, likelihood_variance=False, max_dims:int=-1):
        """
        ...
        if max_dims == -1:
            ...
            selected_dims, _ = get_list_representation(self.m.kernel, max_dims=max_dims, num_dims=num_dims)
    ...
    def plot(
        ...
        """
        if max_dims == -1:
            max_dims = self.m.kernel.max_interaction_depth
        ...
        num_dims = self.num_dims
        selected_dims, _ = get_list_representation(self.m.kernel, num_dims=num_dims, max_dims=max_dims)
        tuple_of_indices = selected_dims[1:]
