...
class OAKKernel(gpflow.kernels.Kernel):
    ...
    def __init__(
        ...
        super().__init__(active_dims=range(num_dims))  # 定义活跃特征 创建核必须的过程……
        ...
        if constrain_orthogonal:
            ...
            for dim in range(len(active_dims)):
                ...
                if (p0[dim] is None) and (p[dim] is None):
                    if empirical_locations[dim] is not None: #经验测度
                    ...
                    elif gmm_measures[dim] is not None:     #高斯混合模型
                        k = OrthogonalRBFKernel(
                            base_kernels[dim](),
                        ...
                        if share_var_across_orders:  # 核方差配置为全为1张量
                            k.base_kernel.variance = tf.ones(
                                1, dtype=gpflow.config.default_float()
                            )

                    if lengthscale_bounds is not None: # 核长度尺度配置
    ...
    def K(self, X, X2=None):
        ...
        if self.share_var_across_orders:
            return reduce(
                ...
                # [self.variances[0]*additive_terms[0]] +
                # [self.variances[1]*additive_terms[1]] +
                # [0] * len(additive_terms[2:])
            )
        else:
            # 只计算常数项、一阶与二阶项
            return reduce(
                tf.add, [self.variances[0] * additive_terms[0]] +
                        [self.variances[1] * additive_terms[1]] +
                        [self.variances[2] * additive_terms[2]]
            )

    def K_diag(self, X): # 得到对角矩阵向量
...
#根据基核（一阶项）获取所有多项式组合,用于计算sobol指数
def get_list_representation(
    ...
    if max_dims == -1:
        max_dims = kernel.max_interaction_depth
    ...
    #添加各个项的组合
    if kernel.max_interaction_depth > 0 and max_dims > 0:
        for ii in range(max_dims + 1):
            if ii > 0:
                tmp = [
                    list(tup) for tup in itertools.combinations(np.arange(num_dims), ii) # 组合
                ]
