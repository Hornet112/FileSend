...
def create_alps_simulator():
    return 1
def Kde(data, percentile, draw: bool=True, name:str = "3sigma"):
    rangel = max(data) - min(data)
    x_min = min(data) - 0 * rangel
    x_max = max(data) + 0 * rangel
    # x_min = min(data)
    # x_max = max(data)
    kde = stats.gaussian_kde(data, bw_method=0.5)
    x = np.linspace(x_min, x_max, 2 * len(data))
    y = kde(x)
